...
public:
    ...
    { // view matrix: translates the center to the origin
        ...
        vec3 v = cross(w, u);
        return TranslateMatrix(wEye * (-1)) * mat4(u.x, v.x, w.x, 0, u.y, v.y, w.y, 0, u.z, v.z, w.z, 0, 0, 0, 0, 1);
    }

    mat4 P()
    { // projection matrix
        return mat4(1 / (tan(fov / 2) * asp), 0, 0, 0, 0, 1 / tan(fov / 2), 0, 0, 0, 0, -(fp + bp) / (bp - fp), -1, 0,
                    0, -2 * fp * bp / (bp - fp), 0);
    }
...
public:
    ...
    VertexData GenVertexData(float u, float v)
    {
        ...
        vd.position = vd.normal = vec3(cosf(u * 2.0f * (float)M_PI) * sinf(v * (float)M_PI),
                                       sinf(u * 2.0f * (float)M_PI) * sinf(v * (float)M_PI), cosf(v * (float)M_PI));
...
public:
    ...
    vec3 point(float u, float v) { return Point(u, v, r); }
};
